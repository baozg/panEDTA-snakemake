...
kmc -fm -k$kmer -t$threads -ci10 -cs100000 $fa ${outpre}_count.kmc tmp/${sample}_SRF
kmc_dump ${outpre}_count.kmc ${outpre}_count.txt
...
tellen=${#tel}
echo $tellen
tidk search -d ./ -s $tel -o ${sample} ${outpre}_SRF.enlong.fa
sed "1d" ${sample}_telomeric_repeat_windows.tsv|awk -v tellen=$tellen '(($3+$4)*tellen)/$2>0.8' |cut -f1 > tel.SRF.list

## rDNA
seqkit grep -v -r -f tel.SRF.list ${outpre}_SRF.enlong.fa > ${outpre}_SRF.enlong_exclude_tel.fa 
barrnap -t $threads --kingdom euk ${outpre}_SRF.enlong_exclude_tel.fa > ${outpre}_SRF.enlong_exclude_tel.rdna.gff3
grep -v "^#" ${outpre}_SRF.enlong_exclude_tel.rdna.gff3|cut -f1|sort|uniq > rDNA.SRF.list

## TE
seqkit grep -v -r -f rDNA.SRF.list ${outpre}_SRF.enlong_exclude_tel.fa > ${outpre}_SRF.enlong_exclude_tel_rdna.fa
TEsorter -dp2 -p ${threads} ${outpre}_SRF.enlong_exclude_tel_rdna.fa
grep -v "^#" ${outpre}_SRF.enlong_exclude_tel_rdna.fa.rexdb.cls.tsv|cut -f1|sort|uniq > TE.SRF.list

# SRF filter
seqkit grep -v -r -f TE.SRF.list ${outpre}_SRF.enlong.fa > ${outpre}_SRF.enlong.TEfree.fa

minimap2 -t $threads -c -N1000000 -f1000 -r100,100 ${outpre}_SRF.enlong.TEfree.fa $fa > ${outpre}_SRF_enlong.TEfree.paf
...
bedtools maskfasta -fi $fa -bed ${outpre}_SRF.mask.bed -fo $sample.masked.SRF.fa
